    ...
    match manifest {
        Some(m) => {
            for (key, offset) in &m.ports {
                // `saturating_add` guards against a wraparound in release
                // builds (batch D #16). With a correctly sized block this can
                // never trigger -- the block is validated to fit the pool and
                // the manifest's offsets -- but never wrap silently past
                // u16::MAX.
                let port = block.0.saturating_add(*offset);
...
PORT=3000\n";
    ...
    fn ends_with_trailing_newline() {
        let rendered = render("p", "/w", (3000, 3004), 1, None, PROJECT_ID, WORKTREE_ID);
    ...
    fn read_block_from_env_round_trips_with_render() {
        ...
        let rendered = render("p", "/w", (3005, 3009), 4, None, PROJECT_ID, WORKTREE_ID);
