...
- **`check`** — validate the config and ledger; exits non-zero on any
  problem (read-only, script-friendly). It also exits non-zero on a
  **degraded backup** — one whose recorded `sequence` is behind the ledger,
  or that is unreadable/corrupt — because a `doctor --repair` from it would
  restore stale state; a merely *missing* backup (a fresh ledger) is only a
  stderr warning, since it heals on the next save.
...
- **`unhook`** — remove portool's `post-checkout` / `post-merge` hook
  content and nothing else. It verifies each hook is actually neutralized
  afterward; if one still invokes portool (a symlink, an unreadable file, or
  a malformed managed block), it prints a machine-readable `partial_unhook`
  JSON object listing the residue and exits non-zero instead of reporting
  success.
- **`deinit`** — full reverse of `init`, run as an ordered transaction:
  remove the hooks, **verify** they no longer invoke portool, remove
  `.env.portool` from every worktree git currently reports *and* every
  worktree path still recorded in the ledger (so it works even when the
  ledger is missing or already empty), release this project's ledger
  allocations, and remove the `info/exclude` entry. If any step can't
  complete — a hook that still invokes portool, an env file that won't
  delete — it prints a `partial_deinit` JSON object listing the residue and
  exits non-zero; when the hooks aren't neutralized it deliberately **keeps**
  the allocations (so a later checkout can't re-hook and hand out a block a
  live env still points at). Pass `--keep-allocations` to only remove the
  hooks and the ignore rule (the pre-0.7 `deinit` behavior).
...
- **An absolute, per-repo (`local`/`worktree` scope) `core.hooksPath` inside
  ...
  exits 0 saying there's nothing to do when the hook is already installed,
  and otherwise points you at the main worktree to run `portool init` in.
  See [Agent-created worktrees](#agent-created-worktrees).
...
- **A `core.hooksPath` that resolves outside this repository** — whatever
  ...
  per-repo hook instead. A *relative* value that escapes into the main
  worktree is refused too: git resolves it against the process's cwd rather
  than a fixed root, so there is no one directory portool can be sure git
  will read.
...
Claude Code checks its background sessions ("task chips", `EnterWorktree`,
`--worktree`) out under `.claude/worktrees/<name>`. It creates them with a
plain `git worktree add`, so **portool's `post-checkout` hook does fire**
and the new worktree gets its own block like any other.

One thing it does afterwards needs a word, because it used to confuse
portool: if your repo sets a *relative* `core.hooksPath` (Husky, lefthook,
…), the harness rewrites it into the new worktree's `config.worktree` as an
**absolute** path pointing at the main worktree — necessary, because git
resolves a relative value against the process's cwd, which a linked
worktree would get wrong.

Since 0.10.1 portool recognizes that shape: from such a worktree, `sync`
and `doctor` report the very hook git runs, and `portool init` no longer
claims the hook is missing. It still never writes into the main worktree's
files from a linked one — install (and `unhook`) the hook in the main
worktree, and git shares it with every worktree of the repository.
