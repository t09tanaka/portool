...
/// Removes the `sequence: N` field from an `.env.portool` `# block:` header
/// (mirrors `src/envfile.rs::strip_sequence`), so tests comparing env bodies
/// can ignore the ledger-global sequence, which changes on every save.
fn strip_sequence_field(content: &str) -> String {
    ...
    assert_eq!(
        ...
        format!(
            "# generated by portool \u{2014} DO NOT EDIT\n\
             # block: {start}-{end}  generation: 1  sequence: 0  project: repo  worktree: {}\n\
             PORTOOL_PROJECT_ID={}\n\
             PORTOOL_WORKTREE_ID={}\n\
             PORT={start}\n",
            worktree_key(&repo),
...
fn ls_table_and_json_shapes() {
    ...
    assert_eq!(json["registry_schema_version"], serde_json::json!(4));
    assert_eq!(
...
fn ls_json_emits_format_v1_envelope() {
    ...
    assert!(alloc["pending_block"].is_null());
    assert_eq!(alloc["state"], "ok");
    assert_eq!(alloc["sync_required"], false);
}
...
fn sync_upgrades_an_old_format_env_file_then_second_sync_is_a_noop() {
    ...
    // Sync must notice the mismatch and add the ID lines back. The `sequence:`
    // header field is re-stamped with the current ledger sequence on this
    // rewrite, so compare with that field stripped from both sides (the ID
    // restoration is what this test is about).
    assert!(env.run(&repo, &["sync"]).status.success());
    let upgraded = fs::read_to_string(repo.join(".env.portool")).unwrap();
    assert_eq!(
        strip_sequence_field(&upgraded),
        strip_sequence_field(&contents),
...
/// P0-2: a backup that is *behind* the ledger (its parsed `sequence` is
/// lower), or one that is unreadable, is a degraded state -- a
/// `doctor --repair` from it would roll allocations back -- so `check` exits
/// non-zero, not with a mere warning. A matching backup keeps exit 0.
#[test]
fn check_fails_when_backup_is_behind_or_corrupt() {
    let env = TestEnv::new();
    let repo = env.path("repo");
    init_repo(&repo);
    assert!(env.run(&repo, &["sync"]).status.success());
    ...
    // A behind backup: a valid ledger whose sequence is lower than main's.
    let mut ledger: serde_json::Value =
        serde_json::from_slice(&fs::read(env.registry_path()).unwrap()).unwrap();
    let main_seq = ledger["sequence"].as_u64().unwrap();
    ledger["sequence"] = serde_json::json!(main_seq.saturating_sub(1));
    fs::write(&bak_path, serde_json::to_vec(&ledger).unwrap()).unwrap();
    let out = env.run(&repo, &["check"]);
    assert!(
        !out.status.success(),
        "a behind backup must be a degraded (non-zero) check"
    );
    assert!(String::from_utf8_lossy(&out.stderr).contains("backup"));

    // An unreadable backup is likewise degraded.
    fs::write(&bak_path, b"{ not json").unwrap();
    assert!(
        !env.run(&repo, &["check"]).status.success(),
        "a corrupt backup must be a degraded (non-zero) check"
    );

    // A matching backup is healthy again (exit 0).
    fs::write(&bak_path, &original_bak).unwrap();
    let out = env.run(&repo, &["check"]);
    assert!(
        out.status.success(),
        "a matching backup must pass: {}",
...
fn deinit_removes_env_files_missing_from_registry() {
    ...
    let wtsync = env.run(&wt, &["sync"]);
    assert!(
        wtsync.status.success(),
        "wt sync stderr: {}",
        String::from_utf8_lossy(&wtsync.stderr)
    );
...
fn future_schema_ledger_is_not_auto_reset() {
    ...
    let dir = env.registry_path().parent().unwrap().to_path_buf();
    fs::create_dir_all(&dir).unwrap();
    let future = br#"{"version":5,"range":[3000,9999],"projects":{},"reservations":[]}"#.to_vec();
...
fn v2_ledger_is_migrated_with_blocks_preserved() {
    ...
    edit_registry(&env, |reg| {
        ...
        reg.as_object_mut().unwrap().remove("sequence");
    ...
    // pending_block is now exposed (null when no move is pending, P0-4).
    assert!(entry["pending_block"].is_null());
}
